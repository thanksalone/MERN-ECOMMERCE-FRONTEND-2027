...
const Search = () => {
  ...
  const {isLoading: productLoading, data: searchData, isError: productIsError, error: productError} = useSearchProductsQuery({
    search,
    sort,
    category,
    page,
  ...
  if (isError) {
   ...
   if (productIsError) {
    const err = productError as customErrorType;
    toast.error(err.data.message);
  };
    ...
    <div className="product-search-page">
      ...
      <main>
        ...
        <article>
          ...
          <button disabled={page == searchData.totalPage} onClick={() => setPage((prev) => prev + 1)}>Next</button>
        </article>
          )
        }
